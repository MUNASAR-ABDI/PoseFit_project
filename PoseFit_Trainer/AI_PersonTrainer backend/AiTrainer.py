...
def main():
            ...
            try:
                ...
                if not user_data.get("verified", False):
                    ...
                    if resend in ('y', 'yes'):
                        ...
                        email_sent = email_sys.send_verification_email(user_email, verification_code)
                            ...
                            for attempt in range(max_attempts):
                                ...
                                if user_code == verification_code:
                                    ...
                                    if user_db and user_email in user_db:
                                        ...
                                        user_db[user_email]["verification_time"] = time.time()
                        ...
                        print("Verification is required to use the application. Please verify your email.")
                        continue
            ...
            try:
                ...
                if user_reg:
                    ...
                    if email_sent:
                            ...
                            for attempt in range(max_attempts):
                                ...
                                if user_code == verification_code:
                                    ...
                                    if user_db and email in user_db:
                                        ...
                                        user_db[email]["verification_time"] = time.time()
                                        db_sys.save_user_database(user_db)
                        ...
                        print("Failed to send verification email. Please try logging in and requesting a new code.")
                    ...
                    for attempt in range(max_attempts):
                        ...
                        if user_code == verification_code:
                            ...
                            while True:
                                ...
                                user_db = db_sys.load_user_database()
                                if user_db and email in user_db:
                                    user_db[email]["hashed_password"] = db_sys.get_password_hash(new_password)
                                    db_sys.save_user_database(user_db)
    ...
    try:
        ...
        congratulations_img = os.path.join(os.path.dirname(os.path.abspath(__file__)), "TrainerImages", "congratulations.jpg")
        # Try alternate images if the main one doesn't exist
        if not os.path.exists(congratulations_img):
            congratulations_img = os.path.join(os.path.dirname(os.path.abspath(__file__)), "TrainerImages", "you_rock.jpeg")
        if not os.path.exists(congratulations_img):
            congratulations_img = os.path.join(os.path.dirname(os.path.abspath(__file__)), "TrainerImages", "great_job2.jpeg")
        if not os.path.exists(congratulations_img):
            congratulations_img = os.path.join(os.path.dirname(os.path.abspath(__file__)), "TrainerImages", "end_screen.jpeg")
            
        final_video_path = session.completion_screen(congratulations_img)
        
        # Record the workout in the user's profile
        if performance:
            record_workout_in_profile(user_profile_manager, user_email, session, performance)
        
        # Handle video and email operations
        handle_workout_completion(final_video_path, session, performance, user_email, first_name)
    ...
    if profile and "body_metrics" in profile:
        ...
        for metric in ["age", "gender", "weight", "height"]:
            if metrics.get(metric):
                ...
                unit = " kg" if metric == "weight" else " cm" if metric == "height" else ""
        ...
        if metrics.get("bmi"):
            ...
            from UserProfile import UserProfileManager
            health_assessment = UserProfileManager().get_health_assessment(metrics["bmi"])
...
def get_workout_settings(coach, user_email):
    ...
    try:
        ...
        choice = input("Enter 'r' for recommended workout or 'c' for custom: ").strip().lower()
    ...
    def mouse_callback(event, x, y, flags, param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            if 0 <= clicked_camera_index_pos < len(available_cameras_indices):
                selected_camera_index = available_cameras_indices[clicked_camera_index_pos]
    ...
    while not break_loop:
        ...
        for i, cap in enumerate(live_camera_feeds):
            ...
            if success:
                ...
                cv2.putText(frame, f"Click to Select Cam {available_cameras_indices[i]}", (10, 20), 
...
def handle_workout_completion(final_video_path, session, performance, user_email, first_name):
    ...
    if final_video_path:
        if not os.path.exists(final_video_path):
            ...
            print("There was an issue with the video recording. Sending stats email instead.")
            ...
            result = session.handle_video_options(final_video_path, user_email, first_name)
...
def send_workout_summary_email(user_email, first_name, performance):
    ...
    if email_result:
        ...
        print("There was an issue sending the email. Please check your internet connection.")
...
def show_next_workout_recommendation(coach, user_email, user_profile_manager):
    ...
    try:
        ...
        print("Based on today's performance, here's what I recommend for your next session:")
