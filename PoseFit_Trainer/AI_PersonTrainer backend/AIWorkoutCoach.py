...
class AIWorkoutCoach:
    ...
    def get_rest_recommendation(self, recommendation_type="between_sets"):
        ...
        if recommendation_type in ["between_sets", "between_exercises"]:
            return self.rest_recommendations[self.current_intensity][recommendation_type]
        return None
    ...
    def add_age_specific_form_advice(self, exercise_name, age, tip):
        ...
        self.exercise_mistakes[exercise_name]["age_specific_tips"][age_group].append(tip)
        return True
    ...
    def add_body_type_advice(self, exercise_name, height, weight, tip):
        ...
        try:
            ...
            self.exercise_mistakes[exercise_name]["body_type_tips"][body_type].append(tip)
            return True
    ...
    def analyze_form(self, pose_landmarks, exercise_name, user_profile=None):
        ...
        if exercise_name.lower() == "squats":
            ...
            if random.random() < 0.2:  # 20% chance to detect this issue for demo purposes
            ...
            if age:
                try:
                    ...
                    if age_int > 60:
                        ...
                        if random.random() < 0.4:  # Higher chance to detect in older adults
                            detected_issues.append("squat depth too low for senior users")
                except (ValueError, TypeError):
                    pass
            ...
            if age:
                try:
                    ...
                    if age_int > 50:
                        ...
                        if random.random() < 0.3:
                            detected_issues.append("wrist position needs adjustment for comfort")
                except (ValueError, TypeError):
                    pass
            ...
            if height and weight:
                try:
                    ...
                    if bmi < 18.5:
                        ...
                        if random.random() < 0.2:
                            detected_issues.append("consider using lighter weights with perfect form")
                except (ValueError, TypeError, ZeroDivisionError):
                    pass
                ...
                try:
                    ...
                    if age_int > 55:
                        ...
                        if random.random() < 0.4:
                            detected_issues.append("consider lower impact version for joint health")
                except (ValueError, TypeError):
                    pass
        
        return detected_issues
        
    def get_difficulty_progression(self, exercise_name, current_performance, user_experience_level):
    ...
    def get_next_workout_recommendation(self, email):
        ...
        try:
            user_recommendation = self.workout_recommender.generate_user_recommendation_message(email)
            ...
            if profile and "body_metrics" in profile:
                ...
                if all([age, gender, weight, height]):
                    ...
                    try:
                        ...
                        # For older users
                        if age_int > 50:
                            additional_tips.append("As you're over 50, make sure to warm up properly and focus on mobility exercises.")
                        
                        # BMI-based recommendations (simplified)
                        if height_float > 0:
                            bmi = weight_float / ((height_float/100) ** 2)
                            if bmi > 25:
                                additional_tips.append("Including more cardiovascular exercise could help with weight management.")
                            elif bmi < 18.5:
                                additional_tips.append("Consider adding strength training to build muscle mass.")
                        
                        # Gender-specific tips
                        if gender.lower() in ['female', 'f', 'woman']:
                            additional_tips.append("Women typically benefit from resistance training to improve bone density.")
                        elif gender.lower() in ['male', 'm', 'man']:
                            additional_tips.append("Men often benefit from including flexibility work in their routine.")
    ...
    def get_exercise_tip(self, detected_issues=None, user_profile=None):
        ...
        if age and self.current_exercise:
            try:
                ...
                age_tips = self.exercise_mistakes.get(self.current_exercise, {}).get("age_specific_tips", {}).get(age_group, [])
        ...
        if height and weight and self.current_exercise:
            try:
                ...
                body_tips = self.exercise_mistakes.get(self.current_exercise, {}).get("body_type_tips", {}).get(body_type, [])
        ...
        available_tips = [tip for tip in self.exercise_mistakes[self.current_exercise] 
                          if isinstance(tip, dict) and "observation" in tip and tip["observation"] not in self.used_tips]
    ...
    def overlay_coach_feedback(self, frame, feedback_text, position=(50, 50), color=(255, 255, 255)):
        ...
        # Add the text
        cv2.putText(frame, feedback_text, position, 
