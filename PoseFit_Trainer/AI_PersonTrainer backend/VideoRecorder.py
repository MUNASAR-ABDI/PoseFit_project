...
class VideoRecorder:
    def __init__(self, temp_folder="temp_videos", output_folder="saved_workouts"):
        ...
        self.session_videos = {}  # Store videos by session ID to ensure we can combine all exercises from a session
        ...
        self.frame_queue = queue.Queue(maxsize=120)  # Store up to 120 frames (5 seconds at 24fps)
        ...
        print(f"[VideoRecorder] Initialized with frame size: {self.frame_width}x{self.frame_height} @ {self.fps}fps, bitrate: {self.bitrate/1000000}Mbps")
    ...
    def start_recording(self, exercise_name, session_id=None):
        ...
        if session_id:
            ...
            if session_id not in self.session_videos:
                ...
                print(f"[VideoRecorder] Created new session videos list for session {session_id}")
            ...
            if session_id not in VideoRecorder._all_session_videos:
                ...
                print(f"[VideoRecorder] Created global session videos list for session {session_id}")
        ...
        if not success:
            try:
                ...
                if self.video_writer.isOpened():
                    ...
                    print(f"[VideoRecorder] Failed to create video writer with mp4v codec")
            except Exception as e:
        ...
        if not success:
            try:
                print(f"[VideoRecorder] Trying XVID codec (will convert to MP4 later)...")
                ...
                if self.video_writer.isOpened():
                    ...
                    print(f"[VideoRecorder] Failed to create video writer with XVID codec")
            except Exception as e:
        ...
        if not success:
            try:
                ...
                if self.video_writer.isOpened():
                    ...
                    print(f"[VideoRecorder] Failed to create video writer with H264 codec")
            except Exception as e:
        ...
        if success:
            ...
            print(f"[VideoRecorder] Started recording {exercise_name} at {self.current_video_path}")
            ...
            if not hasattr(self, 'writer_thread') or not self.writer_thread.is_alive():
                ...
                self.writer_thread = threading.Thread(target=self._process_frame_queue, daemon=True)
        ...
        while self.writer_thread_active:
            if self.buffer_enabled and not self.frame_queue.empty():
                try:
                    ...
                    if self.frames_written > 0 and self.frames_written > expected_frames + 10:
                        # Skip this frame to catch up
                        self.frames_dropped += 1
                        if self.frames_dropped % 10 == 0:  # Log every 10 dropped frames
                            print(f"[VideoRecorder] Dropping frames to maintain timing: {self.frames_dropped} dropped so far")
                    ...
                    if self.video_writer and self.video_writer.isOpened():
                        ...
                        print("[VideoRecorder] Video writer is closed, exiting writer thread")
    ...
    def write_frame(self, frame):
        ...
        try:
            ...
            if self.buffer_enabled:
                try:
                    self.frame_queue.put({"frame": frame, "timestamp": time.time()}, timeout=0.1)
                except queue.Full:
                    self.frames_dropped += 1
                    if self.frames_dropped % 10 == 0:
                        print(f"[VideoRecorder] Frame queue full, dropped {self.frames_dropped} frames")
            else:
                if self.video_writer and self.video_writer.isOpened():
                    self.video_writer.write(frame)
                    self.frames_written += 1
    ...
    def stop_recording(self):
        ...
        try:
            if self.recording and self.video_writer is not None:
                ...
                if queue_size > 0:
                    print(f"[VideoRecorder] Waiting for {queue_size} frames to be processed from queue...")
                    wait_start = time.time()
                    patience_time = 30  # Maximum wait time in seconds
                    
                    # Wait with a timeout
                    while not self.frame_queue.empty() and time.time() - wait_start < patience_time:
                        time.sleep(0.1)
                    
                    if not self.frame_queue.empty():
                        remaining = self.frame_queue.qsize()
                        print(f"[VideoRecorder] Queue still has {remaining} frames after waiting {patience_time}s. Clearing queue.")
                        # Clear the queue
                        while not self.frame_queue.empty():
                            try:
                                self.frame_queue.get_nowait()
                ...
                if hasattr(self, 'writer_thread_active'):
                    ...
                    if hasattr(self, 'writer_thread') and self.writer_thread.is_alive():
                        ...
                        if self.writer_thread.is_alive():
                            print(f"[VideoRecorder] Warning: Writer thread did not exit cleanly")
                ...
                if os.path.exists(self.current_video_path):
                    ...
                    if file_size > 0:
                        print(f"[VideoRecorder] Video recorded successfully: {self.current_video_path} ({file_size} bytes)")
                        ...
                        try:
                            ...
                            if cap.isOpened():
                                ...
                                print(f"[VideoRecorder] Video contains {frame_count} frames, {duration:.1f} seconds at {fps} FPS")
                                ...
                                cap.release()
                                print(f"[VideoRecorder] Verified {valid_frames} valid frames")
                                
                                if valid_frames > 0 or frame_count > 0:
                                    # Add to list of exercise videos
                                    self.exercise_videos.append(self.current_video_path)
                                    print(f"[VideoRecorder] Added to exercise videos list (total: {len(self.exercise_videos)})")
                                    
                                    # Also add to session videos if we have a session ID
                                    if hasattr(self, 'current_session_id') and self.current_session_id:
                                        if self.current_session_id not in self.session_videos:
                                            self.session_videos[self.current_session_id] = []
                                        
                                        self.session_videos[self.current_session_id].append(self.current_video_path)
                                        print(f"[VideoRecorder] Added to session {self.current_session_id} videos (total: {len(self.session_videos[self.current_session_id])})")
                                    
                                    # Update global session videos tracking
                                    if hasattr(VideoRecorder, '_all_session_videos'):
                                        if self.current_session_id not in VideoRecorder._all_session_videos:
                                            VideoRecorder._all_session_videos[self.current_session_id] = []
                                        
                                        VideoRecorder._all_session_videos[self.current_session_id].append(self.current_video_path)
                                        print(f"[VideoRecorder] Added to global session {self.current_session_id} videos (total: {len(VideoRecorder._all_session_videos[self.current_session_id])})")
                                else:
                                    print(f"[VideoRecorder] Warning: No valid frames could be read from video")
                        except Exception as verify_err:
                            print(f"[VideoRecorder] Warning: Could not open video file to verify frame count")
                            print(f"[VideoRecorder] Error verifying video: {verify_err}")
                    else:
                        print(f"[VideoRecorder] Warning: Video file is empty (0 bytes): {self.current_video_path}")
                else:
                    print(f"[VideoRecorder] Warning: Video file was not created: {self.current_video_path}")
                ...
                if hasattr(self, 'recording_start_time') and self.recording_start_time:
                    ...
                    if elapsed > 0 and self.frames_written > 0:
                        ...
                        print(f"[VideoRecorder] Total frames written in this recording: {self.frames_written} at {actual_fps:.1f} FPS")
    ...
    def _convert_to_mp4(self, input_path, output_path):
        ...
        try:
            print(f"[VideoRecorder] Converting {input_path} to {output_path} using ffmpeg...")
            ...
            try:
                    ...
                    try:
                        ...
                        if "h264_videotoolbox" in result.stdout:
                            ...
                            command.extend([
                                ...
                                "-crf", "22",            # Quality level (lower = better quality but larger file)
                ...
                print(f"[VideoRecorder] Running conversion command: {' '.join(command)}")
                ...
                try:
                    ...
                    if process.returncode != 0:
                        ...
                        if "No such file or directory" in stderr_output or "Unknown encoder" in stderr_output:
                            # Try again with basic settings
                            print("[VideoRecorder] Retrying with basic encoder settings...")
                            basic_command = [
                                "ffmpeg",
                                "-i", input_path,
                                "-c:v", "libx264",
                            ...
                            retry_process = subprocess.run(basic_command, capture_output=True, check=False)
                            if retry_process.returncode == 0 and os.path.exists(output_path) and os.path.getsize(output_path) > 0:
                                print(f"[VideoRecorder] Retry succeeded with basic settings")
                                return True
                            else:
                                return False
                        return False
                ...
                if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
                    print(f"[VideoRecorder] ffmpeg conversion successful: {output_path} ({os.path.getsize(output_path)} bytes)")
                    return True
                else:
                    print(f"[VideoRecorder] ffmpeg conversion failed - output file missing or empty")
                    return False
            except Exception as e:
            ...
            print(f"[VideoRecorder] Converting video: {width}x{height} @ {fps}fps, {frame_count} frames")
            ...
            try:
                ...
                print(f"[VideoRecorder] Failed to create output video writer for {output_path}")
            ...
            while True:
                ...
                if frames_processed % 100 == 0 or frames_processed == frame_count:
                    print(f"[VideoRecorder] Conversion progress: {frames_processed}/{frame_count} frames")
            ...
            if output_path.endswith('.avi'):
                ...
                try:
                    ...
                    print(f"[VideoRecorder] Failed to convert AVI to MP4, keeping AVI file")
            
            # Verify output exists and has content
            if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
                print(f"[VideoRecorder] OpenCV conversion successful: {output_path} ({os.path.getsize(output_path)} bytes)")
                return True
            else:
                print(f"[VideoRecorder] OpenCV conversion failed - output file missing or empty")
                return False
    ...
    def combine_videos_for_session(self, session_id):
        ...
        if not session_id:
            print("[VideoRecorder] No session ID provided for combine_videos_for_session")
            return None
        
        print(f"[VideoRecorder] Combining videos for session {session_id} (ENHANCED METHOD)")
        
        # If there's a recording in progress with this session, stop it first
        if self.recording and hasattr(self, 'current_session_id') and self.current_session_id == session_id:
            print(f"[VideoRecorder] Found active recording for session {session_id}, stopping it")
            self.stop_recording()
        ...
        if session_id in self.session_videos and self.session_videos[session_id]:
            print(f"[VideoRecorder] Found {len(self.session_videos[session_id])} videos in local session_videos")
            videos_to_combine.extend(self.session_videos[session_id])
        
        # 2. Next check global session tracking
        if hasattr(VideoRecorder, '_all_session_videos') and session_id in VideoRecorder._all_session_videos:
            global_session_videos = [v for v in VideoRecorder._all_session_videos[session_id] if v not in videos_to_combine]
            if global_session_videos:
                print(f"[VideoRecorder] Found {len(global_session_videos)} additional videos in global session tracking")
        ...
        if os.path.isdir(self.temp_folder):
            for filename in os.listdir(self.temp_folder):
                if (filename.lower().endswith('.avi') or filename.lower().endswith('.mp4')) and session_id in filename:
                    file_path = os.path.join(self.temp_folder, filename)
                    if os.path.isfile(file_path) and os.path.getsize(file_path) > 0 and file_path not in videos_to_combine:
                        videos_to_combine.append(file_path)
                        print(f"[VideoRecorder] Found session video by name matching: {filename}")
        ...
        if not videos_to_combine:
            ...
            if recent_videos:
                print(f"[VideoRecorder] Adding {len(recent_videos)} recent videos as fallback")
        ...
        print(f"[VideoRecorder] Found total of {len(videos_to_combine)} videos for session {session_id}")
        for i, video in enumerate(videos_to_combine, 1):
            if os.path.exists(video):
                print(f"[VideoRecorder] Session video {i}: {video} ({os.path.getsize(video)} bytes)")
            else:
            ...
            if os.path.exists(video_path) and os.path.getsize(video_path) > 0:
                try:
                    ...
                    if cap.isOpened() and int(cap.get(cv2.CAP_PROP_FRAME_COUNT)) > 0:
                        ...
                        print(f"[VideoRecorder] Valid video for combining: {video_path} ({int(cap.get(cv2.CAP_PROP_FRAME_COUNT))} frames)")
                    else:
        ...
        if not valid_videos:
            print(f"[VideoRecorder] No valid videos to combine for session {session_id}")
            return None
        
        # Create the output file
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        final_output_file = os.path.join(self.temp_folder, f"workout_session_{session_id}_{timestamp}.mp4")
        
        # If we have only one valid video, handle it directly
        if len(valid_videos) == 1:
            video_path = valid_videos[0]
            print(f"[VideoRecorder] Only one valid video, using it directly: {video_path}")
            ...
            if not video_path.lower().endswith('.mp4'):
                ...
                if self._convert_to_mp4(video_path, final_output_file):
                    ...
                    print(f"[VideoRecorder] Conversion failed, using original format: {video_path}")
                    return video_path
            else:
        ...
        print(f"[VideoRecorder] Combining {len(valid_videos)} videos into a single file")
        ...
        try:
            ...
            file_list_path = os.path.join(self.temp_folder, f"file_list_{timestamp}.txt")
            ...
            ffmpeg_cmd = [
                ...
                '-safe', '0',          # Don't require safe file paths
                '-i', file_list_path,  # Input file list
                '-c:v', 'libx264',     # Re-encode with H.264 to ensure compatibility
            ...
            if result.returncode != 0:
                print(f"[VideoRecorder] First ffmpeg approach failed, trying alternative method...")
                
                # Create an intermediate file for each video
                temp_files = []
                for i, video in enumerate(valid_videos):
                    temp_file = os.path.join(self.temp_folder, f"temp_{timestamp}_{i}.mp4")
                    temp_cmd = [
                        'ffmpeg',
                        '-y',
                        '-i', video,
                ...
                if temp_files:
                    ...
                    alt_cmd = [
                        ...
                        '-safe', '0',
                        '-i', file_list_path,
                        '-c', 'copy',  # Now we can use copy because the files are normalized
                        final_output_file
                    ]
                    print(f"[VideoRecorder] Running alternative ffmpeg command: {' '.join(alt_cmd)}")
            ...
            if result.returncode == 0 and os.path.exists(final_output_file) and os.path.getsize(final_output_file) > 0:
                ...
                try:
                    ...
                    if verify_result.returncode == 0:
                        ...
                        print(f"[VideoRecorder] Warning: Video may have issues: {verify_result.stderr}")
                except Exception as e:
                    print(f"[VideoRecorder] Error verifying video: {e}")
                
                print(f"[VideoRecorder] Successfully combined videos using ffmpeg: {final_output_file}")
        ...
        for video_path in valid_videos:
            ...
            print(f"[VideoRecorder] Processing video: {video_path} ({video_frames} frames)")
            ...
            while True:
                ...
                if frames_processed % 100 == 0:
                    print(f"[VideoRecorder] Processed {frames_processed}/{video_frames} frames from {os.path.basename(video_path)}")
            
            cap.release()
            print(f"[VideoRecorder] Completed processing video: {video_path} ({frames_processed} frames)")
        ...
        if os.path.exists(final_output_file) and os.path.getsize(final_output_file) > 0:
            try:
                ...
                if final_cap.isOpened():
                    ...
                    final_cap.release()
                    print(f"[VideoRecorder] Successfully combined {len(valid_videos)} videos with {total_frames} total frames (final video: {final_frames} frames)")
                    
                    if abs(final_frames - total_frames) > fps:  # Allow small difference due to container metadata
                        print(f"[VideoRecorder] Warning: Final frame count ({final_frames}) differs from processed frames ({total_frames})")
                        # This could indicate a looping issue, but we'll still return the file
                    
                    return final_output_file
                else:
                    print(f"[VideoRecorder] Error: Could not verify the final video file")
            except Exception as e:
    ...
    def combine_videos(self, session_id=None):
        ...
        if session_id:
            print(f"[VideoRecorder] Using enhanced session-specific video combining for session: {session_id}")
        ...
        if self.exercise_videos:
            print(f"[VideoRecorder] Using exercise_videos list with {len(self.exercise_videos)} videos")
    ...
    def continue_recording_session(self, exercise_name, session_id):
        ...
        """
        try:
            print(f"[VideoRecorder] Continuing recording session {session_id} with exercise: {exercise_name}")
            
            # Check if we're already recording and have the same session ID
            if self.recording and hasattr(self, 'current_session_id') and self.current_session_id == session_id:
                # Stop the current recording first to save it
                self.stop_recording()
                print(f"[VideoRecorder] Stopped current recording for continuation in the same session")
            ...
            if recent_videos:
                ...
                print(f"[VideoRecorder] Added {len(recent_videos)} recent videos to session {session_id}")
    ...
    def _find_recent_videos(self, minutes=30):
        ...
        try:
            ...
            for filename in os.listdir(self.temp_folder):
                file_path = os.path.join(self.temp_folder, filename)
                if os.path.isfile(file_path) and file_path.lower().endswith(('.mp4', '.avi')):
            ...
            if recent_videos:
                print(f"[VideoRecorder] Found {len(recent_videos)} recent videos in the last {minutes} minutes")
    ...
    def cleanup_temp_files(self):
        ...
        try:
            ...
            videos_to_remove = self.exercise_videos.copy() if hasattr(self, 'exercise_videos') else []
                ...
                if os.path.exists(video):
                    try:
                        ...
                        try:
                            ...
                            print(f"[VideoRecorder] File {video} is still being accessed, skipping")
                            continue
                    ...
                    for filename in os.listdir(temp_dir):
                        ...
                        if os.path.isfile(file_path):
                            try:
                                ...
                                print(f"[VideoRecorder] File {filename} is still being accessed, skipping")
                                continue
                                
                            file_age = current_time - os.path.getmtime(file_path)
                            if file_age > 1200 or file_path in videos_to_remove:  # 20 minutes or session file
                                try:
                                    os.remove(file_path)
                                    files_removed += 1
                                    print(f"[VideoRecorder] Removed temp file: {filename}")
                                except Exception as e:
                                    print(f"[VideoRecorder] Error removing temp file {filename}: {e}")
                    
                    print(f"[VideoRecorder] Removed {files_removed} additional files from temp directory")
                    
                    # Create cleanup marker
                    marker_path = os.path.join(temp_dir, f"cleanup_marker_{int(time.time())}.txt")
                    try:
                        with open(marker_path, "w") as f:
                            f.write(f"Cleanup performed at {datetime.datetime.now().isoformat()}")
                    except:
                        pass
                except Exception as e:
    ...
    def save_and_email(self, email, name, video_path):
        ...
        try:
            ...
            if not email or not name:
                print(f"[VideoRecorder] Error: Missing email ({email}) or name ({name})")
                return False
            ...
            if not saved_path:
                print(f"[VideoRecorder] Warning: Could not save video locally, using original path")
            ...
            try:
                ...
                success = EmailingSystem.email_user_with_video(email, name, 0, 0, saved_path)
                if success:
                    print(f"[VideoRecorder] Successfully sent email with video to {email}")
                    return saved_path
                else:
    ...
    def _compress_video(self, video_path):
        ...
        try:
            ...
            if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
                print(f"[VideoRecorder] Video compressed successfully: {output_path} ({os.path.getsize(output_path)} bytes)")
                return output_path
            else:
                print(f"[VideoRecorder] ffmpeg compression failed to produce a valid output file")
    ...
    def _compress_with_opencv(self, input_path, output_path):
        ...
        try:
            ...
            print(f"[VideoRecorder] Compressing video from {width}x{height} to {comp_width}x{comp_height}")
            ...
            if not out.isOpened():
                print(f"[VideoRecorder] Failed to create output video writer for compression")
                cap.release()
                return None
            ...
            while True:
                ...
                if frames_processed % 100 == 0:
                    print(f"[VideoRecorder] Compression progress: {frames_processed}/{frame_count} frames")
            ...
            # Verify output
            if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
                print(f"[VideoRecorder] OpenCV compression successful: {output_path} ({os.path.getsize(output_path)} bytes)")
                ...
                if new_size >= orig_size:
                    print(f"[VideoRecorder] Warning: Compressed file is not smaller ({new_size} vs {orig_size} bytes)")
                    # In this case, we might want to return the original instead
                    try:
                        os.remove(output_path)
                        print(f"[VideoRecorder] Deleted ineffective compressed file: {output_path}")
                    except Exception as e:
                        print(f"[VideoRecorder] Error deleting ineffective compressed file: {e}")
                    return input_path
                
                return output_path
            else:
                print(f"[VideoRecorder] OpenCV compression failed - output file missing or empty")
                return None
    ...
    def force_cleanup_temp_videos(self):
        ...
        print("[VideoRecorder] FORCE CLEANUP: Starting aggressive cleanup of temporary files...")
        try:
            # Clean up temp directory
            temp_dir = os.path.join(self.current_dir, "temp_videos")
            if os.path.exists(temp_dir):
                    ...
                    for filename in os.listdir(temp_dir):
                        ...
                        if os.path.isfile(file_path):
                            try:
                                ...
                                print(f"[VideoRecorder] FORCE CLEANUP: Attempting to delete {filename}")
                                ...
                                try:
                                    ...
                                    os.remove(file_path)
                                    files_removed += 1
                                    print(f"[VideoRecorder] FORCE CLEANUP: Removed temp file: {filename}")
                                except PermissionError:
                                    # If permission error, try to change permissions and retry
                                    try:
                                        print(f"[VideoRecorder] FORCE CLEANUP: Permission issue, trying to adjust permissions for {filename}")
                                        os.chmod(file_path, 0o777)  # Give all permissions
                                        os.remove(file_path)
                                        files_removed += 1
                                        print(f"[VideoRecorder] FORCE CLEANUP: Removed temp file after permission fix: {filename}")
                                    except Exception as chmod_err:
                                        print(f"[VideoRecorder] FORCE CLEANUP: Permission change failed: {chmod_err}")
                                        
                                        # Try using unlink as a last resort
                                        try:
                                            print(f"[VideoRecorder] FORCE CLEANUP: Trying unlink for {filename}")
                                            os.unlink(file_path)
                                            files_removed += 1
                                            print(f"[VideoRecorder] FORCE CLEANUP: Removed temp file with unlink: {filename}")
                                        except Exception as unlink_err:
                                            print(f"[VideoRecorder] FORCE CLEANUP: Unlink also failed: {unlink_err}")
                                            
                                            # If all else fails, schedule the file for deletion on next system boot
                                            if sys.platform == 'win32':
                                                try:
                                                    import subprocess
                                                    print(f"[VideoRecorder] FORCE CLEANUP: Attempting to schedule deletion via system command for {filename}")
                                                    subprocess.call(['cmd', '/c', f'del /F /Q "{file_path}" > nul 2>&1'])
                                                except Exception as cmd_err:
                                                    print(f"[VideoRecorder] FORCE CLEANUP: System command failed: {cmd_err}")
                            except Exception as e:
                                print(f"[VideoRecorder] FORCE CLEANUP: Error removing temp file {filename}: {e}")
                
                    print(f"[VideoRecorder] FORCE CLEANUP: Removed {files_removed} files from temp directory")
                    
                    # Set processed flag to ensure normal cleanup also works in future
                    self._videos_processed = True
                    
                    # Create cleanup marker
                    marker_path = os.path.join(temp_dir, f"force_cleanup_marker_{int(time.time())}.txt")
                    try:
                        with open(marker_path, "w") as f:
                            f.write(f"Forced cleanup performed at {datetime.datetime.now().isoformat()}")
                    except:
                        pass
                    
                    return True
                except Exception as e:
                    print(f"[VideoRecorder] FORCE CLEANUP: Error cleaning up temp directory: {e}")
                    return False
            else:
                print(f"[VideoRecorder] FORCE CLEANUP: Temp directory {temp_dir} does not exist")
                return False
        except Exception as e:
        ...
        try:
            ...
            try:
                try:
                    ...
                    print(f"[VideoRecorder] Calling cleanup API endpoint: {cleanup_url}")
                    response = requests.post(cleanup_url, timeout=10)
                    
                    if response.status_code == 200:
                        print(f"[VideoRecorder] API cleanup successful: {response.json()}")
                    else:
                        print(f"[VideoRecorder] API cleanup failed: {response.status_code}")
                except Exception as req_err:
                    print(f"[VideoRecorder] Error calling cleanup endpoint: {req_err}")
            except ImportError:
                print("[VideoRecorder] Requests library not available, skipping API cleanup")
    ...
    def add_videos_from_previous_session(self, previous_session_videos):
        ...
        try:
            ...
            print(f"[VideoRecorder] Adding {len(previous_session_videos)} videos from previous session")
                    ...
                    if file_size > 0:
                        try:
                            ...
                            if cap.isOpened():
                                frame_count = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
                                if frame_count > 0:
                                    print(f"[VideoRecorder] Valid previous video: {video_path} ({frame_count} frames, {file_size} bytes)")
                                    valid_videos.append(video_path)
                                else:
                                    print(f"[VideoRecorder] Previous video has no frames: {video_path}")
                            else:
                                print(f"[VideoRecorder] Could not open previous video: {video_path}")
                            cap.release()
                        except Exception as e:
                            print(f"[VideoRecorder] Error verifying previous video {video_path}: {e}")
                    else:
                        print(f"[VideoRecorder] Previous video is empty (0 bytes): {video_path}")
                else:
            ...
            print(f"[VideoRecorder] Found {len(valid_videos)} valid videos from previous session")
            ...
            for video_path in valid_videos:
                try:
                    ...
                    if os.path.exists(new_path) and os.path.getsize(new_path) > 0:
                        ...
                        print(f"[VideoRecorder] Copied previous video to: {new_path} ({os.path.getsize(new_path)} bytes)")
                    else:
                        print(f"[VideoRecorder] Failed to copy previous video to: {new_path}")
                except Exception as e:
            ...
            print(f"[VideoRecorder] Added {len(copied_videos)} videos from previous session to exercise_videos list")
            print(f"[VideoRecorder] Current exercise_videos list has {len(self.exercise_videos)} videos")
    ...
    def start_workout_session(self, exercise_name, session_id=None):
        ...
        if not self.workout_active:
            print(f"[VideoRecorder] Starting new workout session for exercise: {exercise_name}")
    ...
    def resume_recording(self):
        ...
        if self.is_paused and self.last_exercise_name:
            print(f"[VideoRecorder] Resuming recording for exercise: {self.last_exercise_name}")
            self.is_paused = False
            self.workout_active = True
            return self.start_recording(self.last_exercise_name, self.current_session_id)
        return False
    ...
    def handle_navigation_away(self):
        ...
        if self.recording and self.workout_active:
            print("[VideoRecorder] User navigated away from workout page, pausing recording")
