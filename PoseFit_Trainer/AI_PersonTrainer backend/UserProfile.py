...
class UserProfileManager:
    def __init__(self):
        self.profiles_folder = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "user_profiles")
    ...
    def create_user_profile(self, email, first_name, last_name, age=None, gender=None, weight=None, height=None):
    ...
    def get_health_assessment(self, bmi):
        ...
        if bmi < 18.5:
            return ("Your BMI indicates you are underweight. This may suggest insufficient calorie intake or other health issues. " 
                   "Consider focusing on nutrient-dense foods and strength training to build muscle mass.")
                   
        elif bmi < 25:
            return ("Your BMI is in the healthy range. Maintain your current habits with a balanced diet and regular exercise. " 
                   "Focus on a mix of cardio and strength training for overall fitness.")
                   
        elif bmi < 30:
            return ("Your BMI indicates you are overweight. Consider increasing physical activity and adjusting your diet. " 
                   "Aim for a moderate calorie deficit and include both cardio and strength training in your routine.")
                   
        else:
            return ("Your BMI indicates obesity, which increases risk for several health conditions. " 
    ...
    def load_profile(self, email):
        ...
        try:
            ...
            if not os.path.exists(profile_path):
                user_data_path = os.path.join(os.path.dirname(self.profiles_folder), "user_data.json")
    ...
    def record_workout(self, email, workout_data):
        ...
        if exercise_counts:
            profile["stats"]["favorite_exercise"] = max(exercise_counts, key=exercise_counts.get)
    ...
    def generate_progress_chart(self, email, metric="calories", last_n_workouts=10):
        ...
        history = sorted(profile.get("workout_history", []), 
                         key=lambda x: x.get("date", ""))[-last_n_workouts:]
        
        if not history:  # If no workout history
            return None
            
        dates = [h.get("date", "").split()[0] for h in history]  # Just get the date part
        ...
        if metric == "calories":
            ...
            values = [h.get("time_elapsed", 0) / 60 for h in history]  # Convert to minutes
    ...
    def calculate_progress(self, email):
        ...
        this_week_workouts = [
            w for w in profile["workout_history"] 
            if datetime.datetime.strptime(w["date"], "%Y-%m-%d %H:%M:%S") >= start_of_week
        ]
        ...
        if this_week_workouts:
            avg_calories = sum(w["calories"] for w in this_week_workouts) / len(this_week_workouts)
            avg_minutes = sum(w["time_elapsed"] for w in this_week_workouts) / len(this_week_workouts) / 60
        
        progress = {
            "workouts": {
                "current": workouts_this_week,
                "goal": weekly_workout_goal,
                "percentage": min(100, int((workouts_this_week / weekly_workout_goal) * 100)) if weekly_workout_goal > 0 else 0
            },
            "calories": {
                "current": avg_calories,
                "goal": calories_goal,
                "percentage": min(100, int((avg_calories / calories_goal) * 100)) if calories_goal > 0 else 0
            },
            "time": {
                "current": avg_minutes,
                "goal": minutes_goal,
                "percentage": min(100, int((avg_minutes / minutes_goal) * 100)) if minutes_goal > 0 else 0
            }
        }
        ...
        try:
            ...
            for metric in metrics:
                try:
                    # Get data for chart
                    history = sorted(profile.get("workout_history", []), 
                                   key=lambda x: x.get("date", ""))[-10:]  # Last 10 workouts
    ...
    def generate_progress_dashboard(self, email):
        ...
        recent_workouts = sorted(history, key=lambda x: x.get("date", ""), reverse=True)[:5]
        
        # Total stats
        total_calories = sum(w.get("calories", 0) for w in history)
        total_time = sum(w.get("time_elapsed", 0) for w in history) / 60  # Convert to minutes
        ...
        recent_workouts_4wk = [w for w in history if w.get("date", "").split()[0] >= recent_date]
        
        if recent_workouts_4wk:
            avg_calories = sum(w.get("calories", 0) for w in recent_workouts_4wk) / len(recent_workouts_4wk)
            avg_time = sum(w.get("time_elapsed", 0) for w in recent_workouts_4wk) / len(recent_workouts_4wk) / 60
            avg_completion = sum(w.get("rep_completion", 0) for w in recent_workouts_4wk) / len(recent_workouts_4wk)
        else:
        ...
        with open(dashboard_path, 'w') as f:
            ...
            for workout in recent_workouts:
                ...
                exercises = ", ".join([ex.get("name", "") for ex in workout.get("exercises", [])])
