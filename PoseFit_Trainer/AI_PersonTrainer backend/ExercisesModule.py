...
def draw_modern_counter(img, text, position, text_color=(255, 255, 255), bg_color=(0, 0, 255), text_size=0.55, text_thickness=2):
    ...
    rect_height = text_height + padding_y * 2 + 2  # Added extra pixels for vertical spacing
    ...
    for i in range(rect_height):
        ...
        gradient_color = np.clip(base_color * (0.8 + 0.4 * (1-gradient_factor)), 0, 255).astype(np.uint8)
        # Draw horizontal line with gradient color
        if i < rect_height and 0 < rect_width-1:  # Avoid out-of-bounds errors
            cv2.line(roi, (0, i), (rect_width-1, i), tuple(map(int, gradient_color)), 1)
    ...
    cv2.line(overlay, (1, rect_height-1), (rect_width-2, rect_height-1), dark_color, 1)
    cv2.line(overlay, (rect_width-2, 1), (rect_width-2, rect_height-1), dark_color, 1)
    ...
    cv2.circle(overlay, (rect_width-radius-2, radius+1), radius, light_color, 1)  # Top-right
    cv2.circle(overlay, (radius+1, rect_height-radius-1), radius, dark_color, 1)  # Bottom-left
    cv2.circle(overlay, (rect_width-radius-2, rect_height-radius-1), radius, dark_color, 1)  # Bottom-right
    ...
    if rect_start[1] + rect_height <= img.shape[0] and rect_start[0] + rect_width <= img.shape[1]:
    ...
    cv2.putText(img, text, (text_x+1, text_y+1), cv2.FONT_HERSHEY_SIMPLEX, text_size, (0, 0, 0), text_thickness)
    cv2.putText(img, text, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, text_size, text_color, text_thickness)
...
class utilities():
    ...
    def illustrate_exercise(self, example, exercise, web_mode=False):
        ...
        while seconds > 0:
            ...
            cv2.putText(img, exercise + " in: " + str(int(seconds)), (350, 50),
    ...
    def display_rep_count(self, img, count, total_reps):
        ...
        draw_modern_counter(img, f"{int(count)}/{total_reps}", (20, 20), 
                           text_color=(255, 255, 255), bg_color=(0, 0, 200))
    ...
    def draw_performance_bar(self, img, per, bar, color, count, is_left=True):
        ...
        cv2.rectangle(img, (bar_left-1, bar_top-1), (bar_right+1, bar_bottom+1), (120, 120, 120), 2)
        
        # Draw background (unfilled portion) with dark gray
        cv2.rectangle(img, (bar_left, bar_top), (bar_right, bar_bottom), (50, 50, 50), cv2.FILLED)
        
        # Draw the filled portion with gradient color
        cv2.rectangle(img, (bar_left, int(bar_fill)), (bar_right, bar_bottom), color, cv2.FILLED)
        
        # Add percentage markers
        for i in range(25, 100, 25):
            marker_y = int(np.interp(i, (0, 100), (bar_bottom, bar_top)))
            cv2.line(img, (bar_left, marker_y), (bar_left + 5, marker_y), (200, 200, 200), 1)
            
        # Add percentage text at top
        text_x = bar_left - 30 if is_left else bar_right + 5
        cv2.putText(img, f'{int(per)}%', (text_x, bar_top - 10), 
...
class simulate_target_exercies():
    ...
    def release_all_cameras(cls):
        ...
        for session_id, session in list(cls._active_sessions.items()):
            try:
                ...
                print(f"[ExercisesModule] Error releasing camera for session {session_id}: {e}")
    ...
    def initialize_video_stream(self):
        ...
        try:
            if not hasattr(self, 'video_stream') or self.video_stream is None:
                print(f"[ExercisesModule] Initializing new VideoStream for camera {self.camera_index}")
                self.video_stream = VideoStream(self.camera_index)
                self.video_stream.start()
                # Wait briefly to ensure stream is running
                time.sleep(1)
                print(f"[ExercisesModule] VideoStream initialized for camera {self.camera_index}")
            else:
                print(f"[ExercisesModule] Using existing VideoStream for camera {self.camera_index}")
                
            # Verify stream is working
            if self.video_stream.more():
                test_frame = self.video_stream.read()
                print(f"[ExercisesModule] Successfully read test frame from VideoStream: {test_frame.shape if test_frame is not None else None}")
            else:
            ...
            try:
                ...
                print(f"[ExercisesModule] Fallback to direct camera access: {ret}, frame shape: {test_frame.shape if ret else None}")
                return None
            except Exception as cap_err:
                print(f"[ExercisesModule] Error with direct camera access too: {cap_err}")
                return None

    def __init__(self, sets=1, reps=6, camera_index=0, exercise_type=None, session_id=None):
        ...
        try:
            ...
            print(f"[ExercisesModule] Successfully initialized VideoRecorder for {exercise_type}")
            
            # Start recording immediately with session ID
            if self.exercise_type:
                print(f"[ExercisesModule] Starting recording for {self.exercise_type} with session ID {self.session_id}")
        ...
        if self.exercise_type:
            ...
            exercise_method = exercise_method_map.get(self.exercise_type, self.exercise_type)
            ...
            if hasattr(self, exercise_method):
                try:
                    ...
                    if callable(method):
                        ...
                        if self.web_mode:
                            self.exercise_thread = threading.Thread(target=method, daemon=True)
                            self.exercise_thread.start()
                        else:
                            method()
                    else:
                        raise ValueError(f"'{exercise_method}' is not a callable method")
                except Exception as e:
                    print(f"Error starting exercise {exercise_method}: {str(e)}")
                    print(f"Available methods: {[m for m in dir(self) if not m.startswith('_') and callable(getattr(self, m))]}")
                    raise ValueError(f"Failed to start exercise {exercise_method}: {str(e)}")
            else:
                ...
                raise ValueError(f"Exercise type '{exercise_method}' not supported. Available: {available_methods}")
    ...
    def get_metrics(self):
        ...
        return {
            ...
            "calories_burned": self.calories_burned,
            "completion_percentage": (self.reps_completed / (self.sets * self.reps)) * 100,
            "feedback": self.feedback
        }
    ...
    def warmup(self):
        try:
            self.utils.illustrate_exercise(os.path.join(os.path.dirname(os.path.abspath(__file__)), "TrainerImages", "skip_illustration.jpeg"), "WARM UP", web_mode=self.web_mode)
            ...
            while self.current_set <= self.sets and not exit_exercise and not self.stopped:
                ...
                while count < total_reps and not exit_exercise and not self.stopped:
                    ...
                    if landmark_list and len(landmark_list) >= min_landmarks_required:
                        ...
                        if all_required_visible:
                            if not detection_ready:
                                ...
                                if stable_detection_frames >= required_stable_frames:
                                    ...
                                    print("Detection stable - counting Warm Up exercises")
                            
                            # Only calculate angles if detection is ready
                            if detection_ready:
                                try:
                                    # Visualize arm joints for better user feedback
                                    left_shoulder = (landmark_list[11][1], landmark_list[11][2])
                                    left_elbow = (landmark_list[13][1], landmark_list[13][2])
                                    left_wrist = (landmark_list[15][1], landmark_list[15][2])
                                    ...
                                    cv2.line(img, left_shoulder, left_elbow, line_color, 2)
                                    ...
                                    if (per >= 85 or per <= 15) and (current_time - last_count_update) >= MIN_TIME_BETWEEN_COUNTS:
                                        color = (0, 255, 0)  # Green at counting points
                                        
                                        # Show visual feedback at maximum curl
                                        if per >= 85:
                                            cv2.putText(img, "ARM BENT!", (img.shape[1]//2-150, 200), 
                                                      cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
                                            print("ARM CURL DETECTED!")
                                        elif per <= 15:
                                            cv2.putText(img, "ARM STRAIGHT!", (img.shape[1]//2-150, 200), 
                                                      cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
                                        ...
                                        if result["count"] != count:  # Only update timestamp if count changed
                                            last_count_update = current_time
                                        count = result["count"]
                                        direction = result["direction"]
                                    
                                    # Calculate overall performance based on rep completion
                                    completion_pct = min(100, (count / total_reps) * 100)
                                    ...
                                    cv2.rectangle(img, (img.shape[1]//2 - 150, 20), (img.shape[1]//2 + 150, 40), (50, 50, 50), cv2.FILLED)
                                    cv2.rectangle(img, (img.shape[1]//2 - 150, 20), (int(img.shape[1]//2 - 150 + completion_pct*3), 40), (0, 255, 0), cv2.FILLED)
                                    cv2.putText(img, f"Performance: {int(completion_pct)}%", (img.shape[1]//2 - 145, 35),
                                              cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
                                    
                                    # Update metrics for web mode
                                    self.reps_completed = count
                                    self.calories_burned = count * 0.5  # Simple calorie calculation
                            ...
                            if stable_detection_frames > 0:
                                stable_detection_frames = max(0, stable_detection_frames - 2)
                            ...
                            if missing_parts:
                                ...
                                cv2.putText(img, missing_text, (img.shape[1]//2 - 200, 70), 
                        ...
                        cv2.putText(img, "Please position yourself in frame", (img.shape[1]//2 - 200, 70), 
                                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                    
                    # Display set and rep counters with modern counters
                    draw_modern_counter(img, f"Set {self.current_set}/{self.sets}", (10, 10), 
                                      text_color=(255, 255, 255), bg_color=(90, 60, 40))
                    draw_modern_counter(img, f"Rep {int(count)}/{total_reps}", (10, 55), 
                                      text_color=(255, 255, 255), bg_color=(100, 170, 85))
                    
                    # Display detection status message
                    if not detection_ready:
                        status_msg = f"Position your body in frame... ({stable_detection_frames}/{required_stable_frames})"
                        cv2.putText(img, status_msg, (img.shape[1]//2 - 200, 30), 
                                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                    
                    # Add text to inform user they can press 'q' to exit
                    cv2.putText(img, "Press 'q' to exit exercise", (img.shape[1] - 230, img.shape[0] - 20),
                    ...
                    if hasattr(self, 'video_recorder') and self.video_recorder is not None:
                        try:
                            ...
                            if self.frames_sent_to_recorder % 100 == 0:
                                print(f"[ExercisesModule] Sent {self.frames_sent_to_recorder} frames to video recorder")
                                
                            # Make a copy of the frame to avoid modification issues
                            img_to_record = img.copy()
                            self.video_recorder.write_frame(img_to_record)
                        except Exception as e:
                            print(f"[ExercisesModule] Error sending frame to video recorder: {e}")
                ...
                if count >= total_reps:
                    ...
                    if self.current_set <= self.sets:
                        ...
                        while break_timer > 0 and not exit_exercise and not self.stopped:
                            success, img = cap.read()
                            if success:
                                img = cv2.resize(img, (980, 550))
                                img = cv2.flip(img, 1)
                                # More appealing break timer display
                                draw_modern_counter(img, f"Break: {break_timer} sec", (img.shape[1]//2 - 80, img.shape[0]//2 - 20), 
                                                  text_color=(255, 255, 255), bg_color=(110, 190, 95), text_size=1.0)
            ...
            calories_burned = int((time_elapsed/60) * 8.5)  # Approx calories/min for warmup
            ...
            if hasattr(self, 'cap') and self.cap is not None:
                self.cap.release()
                self.cap = None
            if not self.web_mode and 'window_name' in locals() and cv2.getWindowProperty(window_name, cv2.WND_PROP_VISIBLE) >= 1:
                cv2.destroyAllWindows()
    ...
    def check_body_in_frame(self, landmarks):
        ...
        for point in required_points:
            if point >= len(landmarks) or landmarks[point][4] < 0.5:  # visibility threshold
                return False
        return True
            ...
            if detector.results and detector.results.pose_landmarks:
                ...
                if hasattr(self, 'user_profile') and self.user_profile:
                    ...
                    if detected_issues:
                        ...
                        form_tip = coach.get_exercise_tip(detected_issues, self.user_profile)
                        if form_tip:
                            print(form_tip)
    ...
    def push_ups(self):
        try:
            self.utils.illustrate_exercise(os.path.join(os.path.dirname(os.path.abspath(__file__)), "TrainerImages", "push_up_illustration.jpeg"), "PUSH UPS", web_mode=self.web_mode)
            detector = pm.posture_detector()
            # Use reps directly without difficulty_level multiplier
            total_reps = self.reps
            ...
            while self.current_set <= self.sets and not exit_exercise and not self.stopped:
                ...
                while count < total_reps and not exit_exercise and not self.stopped:
                    ...
                    if landmark_list and len(landmark_list) >= min_landmarks_required:
                        ...
                        for landmark_id in [12, 14, 16]:
                            ...
                            if visibility >= 0.6:  # Slightly lower threshold for arms
                                right_arm_count += 1
                        right_arm_visible = right_arm_count >= 2  # Need at least 2 points
                                
                        # We need face and at least one arm visible
                        all_required_visible = face_visible and (left_arm_visible or right_arm_visible)
                        ...
                        if all_required_visible:
                            ...
                            if detection_ready:
                                try:
                                    ...
                                    left_vis = detector.get_landmark_visibility(11) + detector.get_landmark_visibility(13) + detector.get_landmark_visibility(15)
                                    right_vis = detector.get_landmark_visibility(12) + detector.get_landmark_visibility(14) + detector.get_landmark_visibility(16)
                                    ...
                                    if use_left_side and left_arm_visible:
                                        ...
                                        shoulder = (landmark_list[12][1], landmark_list[12][2])
                                        elbow = (landmark_list[14][1], landmark_list[14][2])
                                        wrist = (landmark_list[16][1], landmark_list[16][2])
                                    else:
                                        ...
                                        if use_left_side:
                                            ...
                                            if detector.get_landmark_visibility(11) > 0.5 and detector.get_landmark_visibility(13) > 0.5:
                                                shoulder = (landmark_list[11][1], landmark_list[11][2])
                                                elbow = (landmark_list[13][1], landmark_list[13][2])
                                                # If wrist not visible, estimate its position
                                                if detector.get_landmark_visibility(15) > 0.5:
                                                    wrist = (landmark_list[15][1], landmark_list[15][2])
                                                else:
                                                    # Estimate wrist position based on elbow
                                                    dx = elbow[0] - shoulder[0]
                                                    dy = elbow[1] - shoulder[1]
                                                    wrist = (int(elbow[0] + dx * 0.8), int(elbow[1] + dy * 0.8))
                                            else:
                                                raise ValueError("Not enough landmarks for angle calculation")
                                        else:
                                            # Try to use right arm with whatever landmarks are available
                                            if detector.get_landmark_visibility(12) > 0.5 and detector.get_landmark_visibility(14) > 0.5:
                                                shoulder = (landmark_list[12][1], landmark_list[12][2])
                                                elbow = (landmark_list[14][1], landmark_list[14][2])
                                                # If wrist not visible, estimate its position
                                                if detector.get_landmark_visibility(16) > 0.5:
                                                    wrist = (landmark_list[16][1], landmark_list[16][2])
                                                else:
                                                    # Estimate wrist position based on elbow
                                                    dx = elbow[0] - shoulder[0]
                                    ...
                                    try:
                                        ...
                                        cv2.putText(img, f"{int(angle)}°", elbow, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
                                            
                                        print(f"Push-up Angle ({side_name}): {angle:.1f}°")
                                        ...
                                        if (per >= 85 or per <= 15) and (current_time - last_count_update) >= MIN_TIME_BETWEEN_COUNTS:
                                            ...
                                            if per >= 85:
                                                cv2.putText(img, "PUSH-UP DOWN!", (img.shape[1]//2-150, 200), 
                                                          cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
                                                print("PUSH-UP BOTTOM POSITION DETECTED!")
                                            elif per <= 15:
                                                cv2.putText(img, "PUSH-UP UP!", (img.shape[1]//2-150, 200), 
                                                          cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
                                        ...
                                        # Draw performance bars and indicators
                                        utilities().draw_performance_bar(img, completion_pct, bar, color, count, is_left=use_left_side)
                                        ...
                                        self.reps_completed = count
                                        self.calories_burned = count * 0.6  # Simple calorie calculation
                                        self.feedback = "Good form! Keep going!"
                                        
                                        # Add form guidance based on angle
                                        if 30 <= per <= 70:  # Mid-range position
                                            cv2.putText(img, "Keep your back straight", (img.shape[1]//2-150, 240), 
                ...
                if hasattr(self, 'video_recorder') and self.video_recorder:
                    if hasattr(self.video_recorder, 'recording') and self.video_recorder.recording:
    ...
    def bicep_curls(self):
        self.utils.illustrate_exercise(os.path.join(os.path.dirname(os.path.abspath(__file__)), "TrainerImages", "bicep_curls_illustration.jpeg"), "BICEP CURLS", web_mode=self.web_mode)
        detector = pm.posture_detector()
        # Use reps directly without difficulty_level multiplier
        total_reps = self.reps
        ...
        detection_ready = False
        min_landmarks_required = 20  # Minimum number of landmarks to consider a good detection
        ...
        while self.current_set <= self.sets and not exit_exercise and not self.stopped:
            ...
            while (left_count < total_reps or right_count < total_reps) and not exit_exercise and not self.stopped:
                ...
                if landmark_list and len(landmark_list) >= min_landmarks_required:
                    ...
                    if face_visible and (left_arm_visible or right_arm_visible):
                        ...
                        if not detection_ready:
                            ...
                            if stable_detection_frames >= required_stable_frames:
                                detection_ready = True
                                print("Body detection stable - starting to count bicep curls")
                                
                        # Get angles for both arms (swap left/right due to mirroring)
                        left_angle = detector.find_angle(img, 12, 14, 16) if left_arm_visible else 0
                        right_angle = detector.find_angle(img, 11, 13, 15) if right_arm_visible else 0
                        ...
                        left_per = np.interp(left_angle, (170, 40), (0, 100)) if left_arm_visible else 0
                        # Updated right arm angle range to use 343.7° as 100% curl
                        right_per = np.interp(right_angle, (170, 343.7), (0, 100)) if right_arm_visible else 0
                        ...
                        if detection_ready:
                            if left_arm_visible:
                                ...
                                wrist_left = (landmark_list[16][1], landmark_list[16][2])
                                elbow_left = (landmark_list[14][1], landmark_list[14][2])
                                shoulder_left = (landmark_list[12][1], landmark_list[12][2])
                            ...
                            if right_arm_visible:
                                ...
                                wrist_right = (landmark_list[15][1], landmark_list[15][2])
                                elbow_right = (landmark_list[13][1], landmark_list[13][2])
                                shoulder_right = (landmark_list[11][1], landmark_list[11][2])
                                ...
                                # Draw lines connecting joints
                                line_color = (0, 255, 255)  # Yellow color
                                cv2.line(img, shoulder_right, elbow_right, line_color, 2)
                        ...
                        left_bar = np.interp(left_angle, (170, 40), (650, 100)) if left_arm_visible else 650
                        right_bar = np.interp(right_angle, (170, 343.7), (650, 100)) if right_arm_visible else 650

                        current_time = time.time()

                        # Track overall performance as percentage of completed reps
                        left_performance_pct = min(100, (left_count / total_reps) * 100)
                        right_performance_pct = min(100, (right_count / total_reps) * 100)
                        overall_performance_pct = min(100, ((left_count + right_count) / (total_reps * 2)) * 100)
                        ...
                        if detection_ready:
                            ...
                            if left_count < total_reps and left_arm_visible:
                                ...
                                left_color = utilities().get_performance_bar_color(left_per)
                                
                                # Only count at the maximum curl position (>= 85%) or minimum extend position (<= 15%)
                                if (left_per >= 85 or left_per <= 15) and (current_time - last_left_update) >= MIN_TIME_BETWEEN_COUNTS:
                                    left_color = (0, 255, 0)  # Green at counting points
                                    
                                    # Clear visual feedback at maximum curl
                                    if left_per >= 85:
                                        cv2.putText(img, "L: CURL!", (50, 100), 
                                                  cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                                    
                                    result = utilities().repitition_counter(left_per, left_count, left_direction, 'left')
                                    if result["count"] != left_count:  # Only update timestamp if count changed
                                ...
                                left_completion_pct = min(100, (left_count / total_reps) * 100)
                                
                                # Display arm angle percentage and performance bar
                                utilities().draw_performance_bar(img, left_completion_pct, left_bar, left_color, left_count, is_left=True)

                            # Update right arm if visible
                            if right_count < total_reps and right_arm_visible:
                                # Set color based on current curl position
                                right_color = utilities().get_performance_bar_color(right_per)
                                
                                # Only count at the maximum curl position (>= 85%) or minimum extend position (<= 15%)
                                if (right_per >= 85 or right_per <= 15) and (current_time - last_right_update) >= MIN_TIME_BETWEEN_COUNTS:
                                    right_color = (0, 255, 0)
                                    
                                    # Clear visual feedback at maximum curl
                                    if right_per >= 85:
                                        cv2.putText(img, "R: CURL!", (img.shape[1]-150, 100), 
                                                  cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                                    
                                    result = utilities().repitition_counter(right_per, right_count, right_direction, 'right')
                                    if result["count"] != right_count:  # Only update timestamp if count changed
                                ...
                                right_completion_pct = min(100, (right_count / total_reps) * 100)
                                
                                utilities().draw_performance_bar(img, right_completion_pct, right_bar, right_color, right_count, is_left=False)
                                
                            # Draw overall performance based on rep completion
                            cv2.rectangle(img, (img.shape[1]//2 - 150, 20), (img.shape[1]//2 + 150, 40), (50, 50, 50), cv2.FILLED)
                            cv2.rectangle(img, (img.shape[1]//2 - 150, 20), (int(img.shape[1]//2 - 150 + overall_performance_pct*3), 40), (0, 255, 0), cv2.FILLED)
                            cv2.putText(img, f"Performance: {int(overall_performance_pct)}%", (img.shape[1]//2 - 145, 35),
                                      cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
                        else:
                        ...
                        if detection_ready:
                            ...
                            # If both arms are lost, also pause counting
                                ...
                                print("One arm not fully visible - continuing with visible arm only")
                    ...
                    if stable_detection_frames > 0 and not detection_ready:
                        stable_detection_frames = max(0, stable_detection_frames - 2)  # Decrease counter faster than it builds
                ...
                draw_modern_counter(img, f"Set {self.current_set}/{self.sets}", (10, 10), 
                                  text_color=(255, 255, 255), bg_color=(90, 60, 40))
                draw_modern_counter(img, f"L:{int(left_count)}/{total_reps}", (10, 55), 
                                  text_color=(255, 255, 255), bg_color=(75, 80, 195))
                draw_modern_counter(img, f"R:{int(right_count)}/{total_reps}", (10, 100), 
                                  text_color=(255, 255, 255), bg_color=(100, 170, 85))
    ...
    def mountain_climbers(self):
        try:
            self.utils.illustrate_exercise(os.path.join(os.path.dirname(os.path.abspath(__file__)), "TrainerImages", "mountain_climber_illustraion.jpeg"), "MOUNTAIN CLIMBERS", web_mode=self.web_mode)
            detector = pm.posture_detector()
            # Use reps directly without difficulty_level multiplier
            total_reps = self.reps
            ...
            detection_ready = False
            min_landmarks_required = 12  # REDUCED from 15 to 12 for more lenient detection
            ...
            while self.current_set <= self.sets and not exit_exercise and not self.stopped:
                ...
                while (left_count < total_reps or right_count < total_reps) and not exit_exercise and not self.stopped:
                    ...
                    if landmark_list and len(landmark_list) >= min_landmarks_required:
                        ...
                        for landmark_id in [0, 2, 5]:  # Core face landmarks
                            visibility = detector.get_landmark_visibility(landmark_id)
                            if visibility < 0.6:  # IMPROVED: Reduced from 0.7 to 0.6 for more lenient detection
                                face_visible = False
                                missing_parts.append("face")
                                break
                        ...
                        for landmark_id in [24, 26, 28]:
                            visibility = detector.get_landmark_visibility(landmark_id)
                            if visibility < 0.5:  # IMPROVED: Reduced from 0.6 to 0.5 for more lenient detection
                        ...
                        all_required_visible = face_visible and (left_leg_visible or right_leg_visible)
                        
                        # IMPROVED: Add debug prints to see what's happening
                        if not all_required_visible:
                            print(f"[DEBUG] Not all landmarks visible - Face: {face_visible}, Left Leg: {left_leg_visible}, Right Leg: {right_leg_visible}")

                        if all_required_visible:
                            if not detection_ready:
                                stable_detection_frames += 1
                                # IMPROVED: Add debug prints to see progress towards stability
                                print(f"[DEBUG] Stable frames: {stable_detection_frames}/{required_stable_frames}")
                                if stable_detection_frames >= required_stable_frames:
                                    detection_ready = True
                                    print("[DEBUG] Detection stable - counting Mountain Climbers begins!")
                            ...
                            if detection_ready:
                                try:
                                    ...
                                    if left_leg_visible:
                                        hip_left = (landmark_list[24][1], landmark_list[24][2])
                                        knee_left = (landmark_list[26][1], landmark_list[26][2])
                                        ankle_left = (landmark_list[28][1], landmark_list[28][2])
                                        ...
                                        # Draw lines connecting joints
                                        line_color = (0, 255, 255)  # Yellow color
                                        cv2.line(img, hip_left, knee_left, line_color, 2)
                                        cv2.line(img, knee_left, ankle_left, line_color, 2)
                                        
                                        # Calculate angle with explicit error handling
                                        try:
                                            left_angle = detector.find_angle(img, 24, 26, 28)
                                            print(f"Left Leg Angle: {left_angle:.1f}°")
                                        except Exception as e:
                                            print(f"Error calculating left leg angle: {e}")
                                    
                                    if right_leg_visible:
                                        hip_right = (landmark_list[23][1], landmark_list[23][2])
                                        knee_right = (landmark_list[25][1], landmark_list[25][2])
                                        ankle_right = (landmark_list[27][1], landmark_list[27][2])
                                        ...
                                        cv2.line(img, knee_right, ankle_right, line_color, 2)
                                        
                                        # Calculate angle with explicit error handling
                                        try:
                                            right_angle = detector.find_angle(img, 23, 25, 27)
                                            print(f"Right Leg Angle: {right_angle:.1f}°")
                                        except Exception as e:
                                            print(f"Error calculating right leg angle: {e}")
                                    
                                    # Calculate percentages for progress bars - just for visualization
                                    left_per = np.interp(left_angle, (170, 280), (0, 100)) if left_angle != -1 else -1
                                    right_per = np.interp(right_angle, (170, 280), (0, 100)) if right_angle != -1 else -1

                                    # Calculate bar values for visualization
                                    left_bar = np.interp(left_angle, (170, 280), (650, 100)) if left_angle != -1 else 650
                                    right_bar = np.interp(right_angle, (170, 280), (650, 100)) if right_angle != -1 else 650
                                    
                                    # Calculate colors based on current percentage
                                    left_color = utilities().get_performance_bar_color(left_per) if left_per != -1 else default_bar_color
                                    right_color = utilities().get_performance_bar_color(right_per) if right_per != -1 else default_bar_color
                                    ...
                                    if left_count < total_reps and left_leg_visible and left_angle != -1:
                                        # Check if angle is near target
                                        is_at_target = abs(left_angle - LEFT_TARGET) <= LEFT_TOLERANCE
                                        
                                        # Only count if we haven't counted recently (avoid double counting)
                                        if is_at_target and (current_time - last_left_count_time) >= MIN_TIME_BETWEEN_COUNTS:
                                            left_count += 1
                                            print(f"LEFT LEG REP: {left_count} (angle: {left_angle:.1f}°)")
                                            ...
                                            # Show text feedback
                                            cv2.putText(img, "LEFT REP!", (img.shape[1]//4, 120), 
                                                       cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
                                    ...
                                    if right_count < total_reps and right_leg_visible and right_angle != -1:
                                        # Check if angle is near target
                                        is_at_target = abs(right_angle - RIGHT_TARGET) <= RIGHT_TOLERANCE
                                        
                                        # Only count if we haven't counted recently (avoid double counting)
                                        if is_at_target and (current_time - last_right_count_time) >= MIN_TIME_BETWEEN_COUNTS:
                                            right_count += 1
                                            print(f"RIGHT LEG REP: {right_count} (angle: {right_angle:.1f}°)")
                                            ...
                                            # Show text feedback
                                            cv2.putText(img, "RIGHT REP!", (img.shape[1]//2 + img.shape[1]//8, 120), 
                                                       cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
                                    ...
                                    self.calories_burned = (left_count + right_count) * 0.6  # Simple calorie calculation
                                    self.feedback = "Good form! Keep going!"
                                    
                                    # Add form guidance
                                    cv2.putText(img, "Keep your core tight!", (img.shape[1]//2-150, 240), 
                                              cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 200, 200), 2)
                                ...
                                if left_leg_visible:
                                    utilities().draw_performance_bar(img, left_completion_pct, 650, left_color, left_count, is_left=True)
                                if right_leg_visible:
                                    utilities().draw_performance_bar(img, right_completion_pct, 650, right_color, right_count, is_left=False)
                        else:
                            # Some required landmarks are not visible
                            if stable_detection_frames > 0:
                                stable_detection_frames = max(0, stable_detection_frames - 1)
                                print(f"[DEBUG] Landmarks lost, reducing stable frames to: {stable_detection_frames}")
                            ...
                            if detection_ready:
                                if not face_visible:
                                    ...
                                    print("[DEBUG] Lost face tracking - but continuing rep counting")
                                elif not left_leg_visible and not right_leg_visible:
                                    # IMPROVED: Keep detection ready if legs reappear quickly
                                    # detection_ready = False  <- commented out to be more persistent
                                    print("[DEBUG] Lost both legs tracking - but continuing rep counting")
                    ...
                    if not detection_ready:
                        ...
                        cv2.putText(img, "Detection ready! Start moving legs for Mountain Climbers", (img.shape[1]//2 - 250, 30), 
                                  cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            ...
            end_time = time.process_time()
            time_elapsed = end_time - start_time
            calories_burned = int((time_elapsed/60) * 10.5)  # Approx calories/min for mountain climbers (higher intensity)
    ...
    def squats(self):
        try:
            self.utils.illustrate_exercise(os.path.join(os.path.dirname(os.path.abspath(__file__)), "TrainerImages", "squats_illustration.jpeg"), "SQUATS", web_mode=self.web_mode)
            detector = pm.posture_detector()
            # Use reps directly without difficulty_level multiplier
            total_reps = self.reps
                    ...
                    if landmark_list and len(landmark_list) >= min_landmarks_required:
                        ...
                        if all_required_visible:
                            ...
                            if detection_ready:
                                try:
                                    # Visualize leg joints for better user feedback
                                    hip_right = (landmark_list[24][1], landmark_list[24][2])
                                    knee_right = (landmark_list[26][1], landmark_list[26][2])
                                    ankle_right = (landmark_list[28][1], landmark_list[28][2])
                                    ...
                                    try:
                                        ...
                                        if (per >= 85 or per <= 15) and (current_time - last_count_update) >= MIN_TIME_BETWEEN_COUNTS:
                                            ...
                                            if per >= 85:
                                                cv2.putText(img, "SQUAT POSITION!", (img.shape[1]//2-150, 200), 
                                                          cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
                                        ...
                                        # Draw performance bars and indicators
                                        utilities().draw_performance_bar(img, completion_pct, bar, color, count)
            ...
            end_time = time.process_time()
            time_elapsed = end_time - start_time
            calories_burned = int((time_elapsed/60) * 8.5)  # Approx calories/min for squats
    ...
    def release_camera(self):
        ...
        try:
            ...
            if hasattr(self, 'video_recorder') and self.video_recorder:
                if hasattr(self.video_recorder, 'recording') and self.video_recorder.recording:
                    print("[ExercisesModule] Stopping recording before releasing camera")
                    self.video_recorder.stop_recording()
        except Exception as e:
        ...
        if hasattr(self, 'cap') and self.cap is not None:
            try:
                self.cap.release()
                print("[ExercisesModule] Camera resource released via release_camera().")
            except Exception as e:
        ...
        if hasattr(self, 'video_stream') and self.video_stream is not None:
            try:
                ...
                if hasattr(self.video_stream, 'stream') and self.video_stream.stream is not None:
                    self.video_stream.stream.release()
                    print("[ExercisesModule] VideoStream resource released via release_camera().")
            except Exception as e:
    ...
    def start_new_exercise(self, exercise_type, sets, reps):
        ...
        """
        try:
            print(f"[ExercisesModule] Starting new exercise: {exercise_type} in session {self.session_id}")
            ...
            if hasattr(self, 'video_recorder') and self.video_recorder:
                ...
                self.video_recorder.continue_recording_session(exercise_type, self.session_id)
                print(f"[ExercisesModule] Started recording for new exercise {exercise_type} in session {self.session_id}")
                ...
                try:
                    ...
                    if callable(method):
                        ...
                        if self.web_mode:
                            self.exercise_thread = threading.Thread(target=method, daemon=True)
                            self.exercise_thread.start()
                            print(f"[ExercisesModule] Started new exercise thread for {exercise_method}")
                        else:
                            method()
                        return True
                    else:
...
class start_workout_session():
    ...
    def __init__(self, sets=1, reps=6, camera_index=0, enable_recording=True, user_profile=None, exercise_type=None):
        ...
        self.user_profile = user_profile  # Store user profile for enhanced form analysis
        self.exercise_type = exercise_type
        self.video_recorder = None
        self.total_reps = 0
        self.calories_burned = 0
    ...
    def process_frame(self, frame_rgb, pose_landmarks):
        ...
        try:
            ...
            self.total_reps = self.target_exercises.total_reps if hasattr(self.target_exercises, 'total_reps') else 0
            self.calories_burned = self.target_exercises.calories_burned if hasattr(self.target_exercises, 'calories_burned') else 0
            
            # Ensure workout_complete is propagated correctly
            results['workout_complete'] = self.target_exercises.workout_complete if hasattr(self.target_exercises, 'workout_complete') else False
    ...
    def handle_video_options(self, final_video_path, user_email, user_name):
        ...
        try:
            print(f"[WorkoutSession] Handling video options for {user_name} ({user_email}), exercise: {self.exercise_type}")
            ...
            if not os.path.exists(final_video_path):
                print(f"[WorkoutSession] Error: Video file does not exist: {final_video_path}")
                return False
                
            if os.path.getsize(final_video_path) == 0:
                print(f"[WorkoutSession] Error: Video file is empty: {final_video_path}")
                return False
            
            # Send the email with video attachment
            print(f"[WorkoutSession] Sending video to {user_email} for exercise: {self.exercise_type}")
            
            # Always use the save_and_email method regardless of exercise type
            result = self.video_recorder.save_and_email(user_email, user_name, final_video_path)
            ...
            try:
                ...
                print(f"[WorkoutSession] Error during temp video cleanup: {cleanup_err}")
            ...
            if result:
                print(f"[WorkoutSession] Successfully sent video for {self.exercise_type} exercise")
                return result
            else:
