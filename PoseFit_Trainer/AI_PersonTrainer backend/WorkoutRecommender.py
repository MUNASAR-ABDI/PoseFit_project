...
class WorkoutRecommender:
    ...
    def get_user_fitness_level(self, email):
        ...
        avg_completion = sum(w.get("rep_completion", 0) for w in history) / max(1, total_workouts)
    ...
    def infer_user_goal(self, email):
        ...
        strength_focus = sum(exercise_counts.get(ex, 0) for ex in ["Push Ups", "Bicep Curls"])
        endurance_focus = sum(exercise_counts.get(ex, 0) for ex in ["Mountain Climbers"])
    ...
    def get_performance_metrics(self, email):
        ...
        for workout in history:
            for exercise in workout.get("exercises", []):
                ...
                metrics["avg_reps"] = (metrics["avg_reps"] * (metrics["count"] - 1) + 
                                      exercise.get("reps", 0)) / metrics["count"]
                metrics["avg_sets"] = (metrics["avg_sets"] * (metrics["count"] - 1) + 
                                      exercise.get("sets", 0)) / metrics["count"]
                
                # Track completion rate
                if "rep_completion" in exercise:
                    metrics["completion_rate"] = (metrics["completion_rate"] * (metrics["count"] - 1) + 
                                               exercise["rep_completion"]) / metrics["count"]
    ...
    def identify_strengths_weaknesses(self, email):
        ...
        for exercise, metrics in performance.items():
            ...
            progression_score = (metrics["best_reps"] / max(1, metrics["avg_reps"])) * 0.5
            
            # Weight by difficulty
            difficulty = self.exercise_difficulty.get(exercise, 5) / 10
            
            # Calculate overall score
            exercise_scores[exercise] = (completion_score + progression_score) * difficulty
    ...
    def recommend_next_workout(self, email):
        ...
        exercise_pool = list(set(priority_exercises + goal_exercises + list(self.exercise_difficulty.keys())))
    ...
    def _generate_default_workout(self):
        ...
        workout_plan = [
            ...
            {"name": "Squats", "sets": 2, "reps": 8, "rest": progression["rest_between_sets"]},
            {"name": "Push Ups", "sets": 2, "reps": 8, "rest": progression["rest_between_sets"]},
            {"name": "Mountain Climbers", "sets": 2, "reps": 10, "rest": progression["rest_between_sets"]}
        ]
    ...
    def generate_user_recommendation_message(self, email):
        ...
        message += f"Goal: {recommendation['fitness_goal'].replace('_', ' ').capitalize()}\n"
        ...
        if recommendation["focus_areas"]:
            message += "\nFocus Areas: " + ", ".join(area.capitalize() for area in recommendation["focus_areas"])
