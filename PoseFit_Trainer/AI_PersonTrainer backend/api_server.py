...
class RateLimiter:
    ...
    async def __call__(self, request: Request, call_next: Callable):
        ...
        self.requests[ip] = [
            req_time for req_time in self.requests[ip] if now - req_time < self.window
        ]
...
app.mount(
    "/static",
    CachedStaticFiles(directory=os.path.join(current_dir, "static")),
    name="static",
)
...
async def start_workout(
    workout: WorkoutStartRequest, current_user: dict = Depends(get_current_user)
):
    try:
        # Normalize exercise name (replace hyphens with underscores)
        exercise_name = workout.exercise.replace("-", "_")
        logger.info(
            f"Starting workout: {exercise_name} with {workout.sets} sets and {workout.reps} reps"
        )
        ...
        return {"session_id": session_id, "stream_url": f"/workout-stream/{session_id}"}
    except ValueError as ve:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to start workout: {str(e)}"
        )
...
async def process_frame(session_id: str = Form(...)):
    try:
        ...
        if not workout_session:
            return JSONResponse(
                status_code=404, content={"error": "Workout session not found"}
            )
        ...
        return JSONResponse(
            status_code=500, content={"error": f"Failed to process frame: {str(e)}"}
        )
...
async def generate_frames(session):
    ...
    try:
        ...
        frame_interval = float(
            os.getenv("FRAME_INTERVAL", "0.033")
        )  # ~30 FPS by default
        downscale_factor = float(
            os.getenv("DOWNSCALE_FACTOR", "1.0")
        )  # No downscaling by default
            ...
            if "processed_image" in metrics:
                ...
                if downscale_factor != 1.0:
                    try:
                        ...
                        if img is not None:
                            h, w = img.shape[:2]
                            new_h, new_w = int(h / downscale_factor), int(
                                w / downscale_factor
                            )
                            img = cv2.resize(
...
async def workout_page(
    ...
    return templates.TemplateResponse(
        "workout.html",
        {"request": request, "exercise": exercise, "sets": sets, "reps": reps},
    )
...
async def pause_workout(session_id: str = Form(...)):
    try:
        ...
        if (
            session
            and hasattr(session, "target_exercises")
            and session.target_exercises is not None
        ):
            ...
            return {
                "message": "Workout session paused and camera released successfully"
            }
...
async def save_and_email_video(
    request: Request,
    session_id: str = Form(...),
    ...
    try:
        ...
        update_task_progress(
            task_id, 10, "processing", "Initializing video recorder..."
        )
        ...
        if not hasattr(session, "video_recorder") or session.video_recorder is None:
            ...
            try:
                ...
                session.video_recorder = VideoRecorder(
                    progress_callback=lambda p, msg: update_task_progress(
                        task_id, 10 + int(p * 0.6), "processing", msg
                    )
                )
                ...
                update_task_progress(
                    task_id, 0, "error", f"Failed to create video recorder: {str(e)}"
                )
        ...
        if (
            hasattr(session.video_recorder, "recording")
            and session.video_recorder.recording
        ):
            ...
            try:
                ...
                update_task_progress(
                    ...
                    20,
                    "warning",
                    f"Warning: Error stopping recording: {str(e)}",
                )
        ...
        if not final_video_path or not os.path.exists(final_video_path):
            ...
            return {
                "error": "No video file available. Please ensure a workout was recorded.",
                "task_id": task_id,
            }
        ...
        if os.path.getsize(final_video_path) == 0:
            ...
            return {
                "error": "Video file is empty. Please try recording again.",
                "task_id": task_id,
            }

        logger.info(
            f"Successfully combined videos to: {final_video_path} ({os.path.getsize(final_video_path)} bytes)"
        )
        ...
        try:
            ...
            if saved_path:
                ...
                logger.info(
                    "Failed to save video locally, continuing with original path"
                )
            ...
            update_task_progress(
                ...
                70,
                "warning",
                f"Warning: Could not save locally: {str(save_err)}",
            )
        ...
        if hasattr(session, "handle_video_options"):
            try:
                ...
                if result:
                    ...
                    update_task_progress(
                        task_id, 100, "completed", "Video saved and sent successfully"
                    )
                    return {
                ...
                update_task_progress(
                    task_id, 80, "warning", f"Warning: Error in video options: {str(e)}"
                )
        ...
        try:
            ...
            if result:
                ...
                if hasattr(session, "video_recorder") and session.video_recorder:
                    ...
                    try:
                        update_task_progress(
                            task_id, 90, "processing", "Cleaning up temporary files..."
                        )
                        ...
                        update_task_progress(
                            ...
                            90,
                            "warning",
                            f"Warning: Cleanup issue: {str(cleanup_err)}",
                        )

                    # Also force cleanup for maximum effectiveness
                    try:
                        force_cleanup_result = (
                            session.video_recorder.force_cleanup_temp_videos()
                        )
                ...
                update_task_progress(
                    task_id, 90, "partial", "Video saved but email failed"
                )
                return {
            ...
            if saved_path and os.path.exists(saved_path):
                update_task_progress(
                    ...
                    80,
                    "partial",
                    f"Video saved but not emailed: {str(email_err)}",
                )
                return {
                ...
                update_task_progress(
                    task_id, 0, "error", f"Failed to send email: {str(email_err)}"
                )
                return {
...
async def continue_workout(
    ...
    try:
        logger.info(
            f"Continue workout for session: {session_id} with new exercise: {exercise}"
        )
        ...
        logger.info(
            f"Continuing with exercise: {exercise_name}, {sets} sets, {reps} reps"
        )
...
async def cleanup_temp_videos():
    ...
    try:
        ...
        try:
            ...
            if result:
                return {
                    "message": "Successfully cleaned up temp_videos directory",
                    "success": True,
                }
            else:
                return {
            ...
            try:
                marker_path = os.path.join(
                    temp_dir, f"api_cleanup_{int(time.time())}.txt"
                )
            ...
            return {
                "message": f"Manual cleanup completed. Removed {files_removed} files.",
                "success": True,
            }
    except Exception as e:
...
async def request_password_reset(request: ResetPasswordRequest = Body(...)):
    try:
        ...
        try:
            logger.info(
                f"Sending password reset email to {clean_email} with code {reset_code}"
            )
...
async def reset_password(request: PasswordReset = Body(...)):
    try:
        ...
        if (
            not stored_reset_code
            or stored_reset_code != request.reset_code
            or current_time > reset_code_expiry
        ):
...
if __name__ == "__main__":
    ...
    if not is_port_available(port):
        logger.warning(
            f"Port {port} is not available, trying to find an available port..."
        )
