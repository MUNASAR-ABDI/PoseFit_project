...
def get_email_credentials():
    ...
    print(
        "[EmailingSystem] Please set the GMAIL_APP_PASSWORD and SENDER_EMAIL environment variables or create a credentials file."
    )
    return None
...
def send_smtp_email(recipient, message):
    ...
    try:
        ...
        if not creds:
            print(
                "[EmailingSystem] ERROR: No email credentials available, cannot send email"
            )
            return False
        ...
        try:
            ...
            print(
                f"[Email System] Error message: {e.smtp_error.decode() if isinstance(e.smtp_error, bytes) else e.smtp_error}"
            )
            print(
                "\n[Email System] This is likely due to an invalid app password. Please update your Gmail App Password."
            )
            print(
                "[Email System] Instructions: Go to your Google Account > Security > App passwords to generate a new one"
            )
            return False
...
def email_user(email, name, calories, time, rep_completion=None):
    ...
    try:
        ...
        html_content = create_workout_summary_html(
            name, today_date, calories_formatted, time_formatted, rep_completion
        )
...
def email_user_with_video(
    recipient_email, recipient_name, calories, time_elapsed, video_path=None
):
    """
    ...
    if video_path:
        try:
            ...
            if file_size == 0:
                print(
                    f"[EmailingSystem] Warning: Video file exists but is empty: {video_path}"
                )
                # Continue without attachment
                video_path = None
            elif file_size > 25 * 1024 * 1024:  # 25MB max for most email servers
                print(
                    f"[EmailingSystem] Warning: Video file is too large for email attachment: {file_size} bytes"
                )
                # Try to compress the video or offer an alternative
                try:
                    import subprocess

                    compressed_path = video_path.replace(
                        ".mp4", "_email_compressed.mp4"
                    )
                    if video_path.endswith(".avi"):
                        compressed_path = video_path.replace(
                            ".avi", "_email_compressed.mp4"
                        )
                    ...
                    print(
                        f"[EmailingSystem] Attempting to compress video for email: {' '.join(ffmpeg_cmd)}"
                    )
                    result = subprocess.run(ffmpeg_cmd, check=True, capture_output=True)

                    if (
                        os.path.exists(compressed_path)
                        and os.path.getsize(compressed_path) < 20 * 1024 * 1024
                    ):
                        print(
                            f"[EmailingSystem] Using compressed video for email: {compressed_path} ({os.path.getsize(compressed_path)} bytes)"
                        )
                        video_path = compressed_path
                    else:
                        print(
                            "[EmailingSystem] Compression failed or file still too large, will send email without attachment"
                        )
                        # Continue without attachment
                        video_path = None
                except Exception as e:
    ...
    try:
        ...
        if (
            ...
            print(
                f"[EmailingSystem] Attaching video: {video_path} ({os.path.getsize(video_path)} bytes)"
            )
            try:
            ...
            print(
                "[EmailingSystem] No video attached to email (not provided or invalid)"
            )
        ...
        try:
            print(
                f"[EmailingSystem] Connecting to SMTP server: {smtp_server}:{smtp_port}"
            )
            ...
            try:
                ...
                print(
                    f"[EmailingSystem] Resolved SMTP server {smtp_server} to IP: {smtp_ip}"
                )
                server = smtplib.SMTP(smtp_ip, smtp_port, timeout=30)
            except socket.gaierror:
                # If hostname resolution fails, try direct IP or fall back to hostname
                print(
                    f"[EmailingSystem] Failed to resolve hostname, trying direct connection"
                )
                server = smtplib.SMTP(smtp_server, smtp_port, timeout=30)
